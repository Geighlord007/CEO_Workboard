...
| **采购** | 供应商 + 跟进 | 名称 → 类别 → 阶段 → **NDA** → 能力 → 金额 → 开始/结束 → 账期 → 风险 → 联系人 → 最近反馈 |
| **融资** | 投资人/基金（按轮次）+ 跟进 | 机构名 → 轮次 → 阶段 → 赛道 → 地区 → 偏好/支票 → **首次接触 → 最近沟通 → 沟通记录 → 下一步** → 对接人 → 职级 → 邮箱/验证 → 引荐人 → 备注 |
...
搜索、多条件筛选（含 tags 多标签）、点列头排序、列显示/排序记忆、**列宽拖拽记忆**、**表内滚动（横/竖滚动条常驻视口）**、行内编辑、批量选择、分页、密度切换、**导出 CSV（当前筛选 / 全量）**、详情抽屉、空状态与加载骨架。
**跟进体系**：`followups` 表（entityType+entityId 挂回客户/供应商/投资人/人脉四表），各模块「跟进」子页 + 行抽屉「记跟进」+ 看板「最近跟进」卡。
表头与枚举标签**中英双语**（`contracts/crm.ts`：`STAGE_LABELS` / `STAGE_LABELS_EN` / `CONTACT_ROLE_META` / `OUTREACH_STAGE_META`）。

`/data` 数据页（仅 admin）：任意表原始浏览，中文列名、内部字段默认隐藏、同样支持搜索/筛选/导出与白名单字段行内编辑。页顶横幅说明：与 CRM 同一批表、实时双向同步。
...
- **v2.1（跟进体系 + 表格体验）** — 新建 `followups` 跟进表（外键挂回客户/供应商/投资人/人脉）：各模块「跟进」子页、行抽屉「记跟进」、看板「最近跟进」卡；看板「客户推进」改读商机表（仅进行中商机）；删除 v1 遗留子页入口（融资推进/客户卡/商机管道/供应商管理/旧跟进队列）；供应商类别重归类（仪器CRO/CMO·湿实验·小试工艺开发，其他清零）；人脉加 方向标签/机构 筛选；DataTable 列宽拖拽 + 表内滚动（滚动条常驻）；数据页加同源说明横幅
